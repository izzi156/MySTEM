...
class Home extends CI_Controller {
    ...
    public function profile($param1 = "") {
        if ($this->session->userdata('user_login', 'student_login') != true) {
            redirect(site_url('home'), 'refresh');
        }
        ...
        if ($this->session->userdata('user_login') != 1){
            if ($this->session->userdata('admin_login') != 1){
                if ($this->session->userdata('student_login') != 1){
                    echo '<script>alert("You have not sign in!");</script>';
                    redirect('home', 'refresh');
                }
            }
        }
    ...
    public function homework() {
        ...
        $page_data['page_name'] = 'homeworks_page';
        $page_data['page_title'] = get_phrase('homework');
        $this->load->view('frontend/'.get_frontend_settings('theme').'/index', $page_data);
    }

    public function virtual_lab() {
        if ($this->session->userdata('user_login') != 1){
            if ($this->session->userdata('admin_login') != 1){
                if ($this->session->userdata('student_login') != 1){
                    echo '<script>alert("You have not sign in!");</script>';
        ...
        $page_data['page_name'] = 'virtual_lab';
        $page_data['page_title'] = get_phrase('virtual_lab');
        $this->load->view('frontend/'.get_frontend_settings('theme').'/index', $page_data);
    }
    ...
    public function login() {
        ...
        }elseif ($this->session->userdata('student_login')) {
            redirect(site_url('user'), 'refresh');
        }
