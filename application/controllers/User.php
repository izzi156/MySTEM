...
class User extends CI_Controller {
    ...
    public function students($param1 = "", $param2 = "") {
        ...
        if ($param1 == "add") {
          $this->user_model->add_student();
          redirect(site_url('user/students'), 'refresh');
        }
        elseif ($param1 == "edit") {
          $this->user_model->edit_student($param2);
          redirect(site_url('user/students'), 'refresh');
        }
        ...
        $page_data['students'] = $this->user_model->get_student($param2);
      ...
      public function student_form($param1 = "", $param2 = "") {
        ...
        if ($param1 == 'add_student_form') {
          $page_data['page_name'] = 'student_add';
          $page_data['page_title'] = get_phrase('student_add');
          $this->load->view('backend/index', $page_data);
        }
        elseif ($param1 == 'edit_student_form') {
    ...
    public function homeworks() {
        ...
        $page_data['homeworks']=$this->crud_model->get_homeworks();
        $page_data['page_name']= 'homeworks';
        $page_data['page_title']= get_phrase('homeworks');
        $this->load->view('backend/index', $page_data);
    }

    public function homework_actions($param1 = "", $param2 = "") {
        if ($this->session->userdata('user_login') != true) {
            redirect(site_url('login'), 'refresh');
        }

        if ($param1 == "add") {
            $this->crud_model->add_homework();
            redirect(site_url('user/homeworks'), 'refresh');

        }
        elseif ($param1 == "edit") {
            $this->is_the_homework_belongs_to_current_instructor($param2);
    ...
    public function homework_form($param1 = "", $param2 = "") {

        if ($this->session->userdata('user_login') != true) {
            redirect(site_url('login'), 'refresh');
        }

        if ($param1 == 'add_homework') {
            $page_data['languages']	= $this->get_all_languages();
            $page_data['categories'] = $this->crud_model->get_categories();
            $page_data['page_name'] = 'homework_add';
            $page_data['page_title'] = get_phrase('add_homework');
            $this->load->view('backend/index', $page_data);

        }elseif ($param1 == 'homework_edit') {
            $this->is_the_homework_belongs_to_current_instructor($param2);
            $page_data['page_name'] = 'homework_edit';
            $page_data['course_id'] =  $param2;
            $page_data['page_title'] = get_phrase('edit_homework');
            $page_data['languages']	= $this->get_all_languages();
            $page_data['categories'] = $this->crud_model->get_categories();
            $this->load->view('backend/index', $page_data);
        }
    ...
    //check homework belong to current logged in instructor
    function is_the_homework_belongs_to_current_instructor($homework_id) {
        $homework_details = $this->crud_model->get_homework_by_id($homework_id)->row_array();
        if ($homework_details['user_id'] != $this->session->userdata('user_id')) {
            $this->session->set_flashdata('error_message', get_phrase('you_do_not_have_right_to_access_this_course'));
            redirect(site_url('user/homeworks'), 'refresh');
        }
    }
