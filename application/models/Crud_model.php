...
class Crud_model extends CI_Model {
    ...
    public function update_student_settings() {
        $data['value'] = html_escape($this->input->post('allow_student'));
        $this->db->where('key', 'allow_student');
        $this->db->update('settings', $data);
    }
    ...
    public function get_course_video_url($course_id, $param1 = ""){
        // $this->db->where('id', $course_id);
        $this->db->get_where('course', array('id' => $course_id, 'video_url' => $param1));
        return $param1;
    }
    ...
    //homework CRUD
    public function add_homework($param1 = "") {

        $data['title'] = html_escape($this->input->post('title'));
        $data['description'] = $this->input->post('description');
        $data['quiz_url'] = html_escape($this->input->post('quiz_url'));
        $data['date_added'] = strtotime(date('D, d-M-Y'));
        $data['user_id'] = $this->session->userdata('user_id');
        ...
        // Create folder if does not exist
        if (!file_exists('uploads/thumbnails/homework_thumbnails')) {
            mkdir('uploads/thumbnails/homework_thumbnails', 0777, true);
        }

        if ($_FILES['homework_thumbnail']['name'] != "") {
            move_uploaded_file($_FILES['homework_thumbnail']['tmp_name'], 'uploads/thumbnails/homework_thumbnails/'.$homework_id.'.jpg');
            $this->session->set_flashdata('flash_message', get_phrase('course_added_successfully'));
        }
    }

    public function get_homeworks($homework_id = "", $instructor_id = 0) {
        if ($homework_id > 0 && $instructor_id > 0) {
            return $this->db->get_where('homework', array('id' => $homework_id, 'user_id' => $instructor_id));
        } else {
    ...
    public function get_homework_by_id($homework_id = "") {
        //$homework_details = $this->get_course_by_id($homework_id)->row_array();
        return $this->db->get_where('homework', array('id' => $homework_id));
    }
