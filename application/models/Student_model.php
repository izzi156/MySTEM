...
class Student_model extends CI_Model {
    ...
    public function get_user_details() {
        return $this->db->get_where('users', array('role_id' => 2));
    }

    public function get_student($user_id = 0) {
        if ($user_id > 0) {
    ...
    public function get_all_student($user_id = 0) {
        if ($user_id > 0) {
            $this->db->where('id', $user_id);
        }
        return $this->db->get('users');
    ...
    public function add_student() {
        ...
        if ($validity == false) {
            ...
            $this->upload_student_image($user_id);
    ...
    public function edit_student($user_id = "") { // Teacher/user does this editing
        ...
        if ($validity) {
            ...
            $this->upload_student_image($user_id);
            $this->session->set_flashdata('flash_message', get_phrase('student_update_successfully'));
        }else {
            $this->session->set_flashdata('error_message', get_phrase('email_duplication'));
        }
    ...
    public function upload_student_image($user_id) {
        if (isset($_FILES['student_image']) && $_FILES['student_image']['name'] != "") {
            move_uploaded_file($_FILES['student_image']['tmp_name'], 'uploads/student_image/'.$user_id.'.jpg');
            $this->session->set_flashdata('flash_message', get_phrase('student_update_successfully'));
        }
    }
    ...
    public function update_account_settings($user_id) {
        ...
        if ($validity) {
            if (!empty($_POST['current_password']) && !empty($_POST['new_password']) && !empty($_POST['confirm_password'])) {
                $user_details = $this->get_student($user_id)->row_array();
    ...
    public function change_password($user_id) {
        $data = array();
        if (!empty($_POST['current_password']) && !empty($_POST['new_password']) && !empty($_POST['confirm_password'])) {
            $user_details = $this->get_all_students($user_id)->row_array();
            $current_password = $this->input->post('current_password');
            $new_password = $this->input->post('new_password');
            $confirm_password = $this->input->post('confirm_password');
    ...
    public function get_user($id = 0) {
        if ($id > 0) {
            return $this->db->get_all_student($id);
        }else {
            if ($this->check_if_student_exists()) {
    ...
    public function get_student_image_url($user_id) {

         if (file_exists('uploads/student_image/'.$user_id.'.jpg'))
             return base_url().'uploads/student_image/'.$user_id.'.jpg';
        else
